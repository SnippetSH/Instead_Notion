...
export const HomeView = ({ name, KKEUJEOK, isSignedIn }: HomeProps) => {
  const theme = ThemeStore(state => state.theme)
  const navigate = useNavigate()

  const buttonClass = ``
  const contentClass = `${theme === 'light' ? 'bg-white-600' : 'bg-dark-800/50 shadow-white-100/15'} p-10  shadow-lg rounded-lg`
  return (
          ...
          <Content className={contentClass}>
            ...
            <div className="flex text-center space-x-16 w-0.75 justify-center">
              <div className="flex-col items-center justify-center space-y-3">
                ...
                <Button className={buttonClass}>New</Button>
              </div>
